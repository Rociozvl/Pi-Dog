...
const validate= (form) =>{
    ...
    if(!form.life_span){
        errors.life_span = "Life is required,type only numbers separated by a dash (-)"
    }
    return errors
}
...
export default function CreatingDog(){
    ...
    const handleSelect = (e) =>{
        setForm({
            ...form,
            tempeaments:[...form.temperaments, e.target.value]
...
return(
    ...
    <div className={CreatingDogCSS.errors}>{errors.name && <p>{errors.name}</p>}</div>
    <div className={CreatingDogCSS.weight_height_container}>
        ...
        <div className={CreatingDogCSS.error}>{errors.weight && <p>{errors.weight}</p>}</div>
         <div className={CreatingDogCSS.name_lifespan_img_container}>
             ...
             name="life_span"
             onChange={(e)=> handleChange(e)}
             placeholder="Expected lifespan: 14 - 16..."
    ...
    <div className={CreatingDogCSS.selected_temps_container}>
        {form.temperaments.map(e =>
            <div className={CreatingDogCSS.added_temps}>
            <p>{e}</p>
            <div className={CreatingDogCSS.temps_btn_overlay}>
            <button className={CreatingDogCSS.temps_btn} onClick={()=> handleDelete(e)}> <i clas="gg-close-o"></i></button>
             </div>
            </div>
            )}
         </div>
